...
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-strompris-card.svg" width="28" align="absmiddle"> | `ki-strompris-card` | KI Strømpris | Strømpriser for i dag og i morgen i én graf (48 t), med Totalpris / Spotpris / Norgespris og dra for å se hver time |
...
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-strom-card.svg" width="28" align="absmiddle"> | `ki-strom-card` | KI Strøm | Animert hus med strøm fra nettet, prisnivå, effekttrinn og dagens spotpris |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-strom-detaljer-card.svg" width="28" align="absmiddle"> | `ki-strom-detaljer-card` | KI Strøm-detaljer | Seks kompakte detaljkort valgt med `vis:`: strømregning, effekttrinn, effektledd per måned (trykkbare søyler), Norgespris spart, spotpris mot Norgespris og strømregning, som nå har fanene Dag/Uke/Måned/År og en kalenderknapp øverst (`skjul: [uke, kalender]` fjerner enkeltfaner, `faner: false` fjerner hele rada, `faner: i` legger den på kortflaten, `faner_bakgrunn:` maler stripa den står på, og `fane:` velger hvilken den åpner på). Alle sensorene har standardverdier og kan overstyres under `sensorer:` |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-kalender-card.svg" width="28" align="absmiddle"> | `ki-kalender-card` | KI Kalender | Månedskalender med hendelsene fra kalenderne dine: prikk per hendelse i fargen til kalenderen, filterpiller for å slå kalendere av og på, og hendelsene for valgt dag under. Uten `kalendere:` tas alle `calendar.*` |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-basseng-hero-card.svg" width="28" align="absmiddle"> | `ki-basseng-hero-card` | KI Basseng (animert) | Bassenget fra siden: vannflaten bølger og får farge etter temperaturen, bobler når pumpa går, varmepumpa snurrer og blåser varme (saktere i stillemodus), damp når det varmes, lyset lyser opp vannet, og gult varsel når varmepumpa står i auto |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-veggpanel-card.svg" width="28" align="absmiddle"> | `ki-veggpanel-card` | KI Veggpanel | Hele veggpanelet for et rom som ett kort, laget for iPad: toppstripe med klokke og status, scener, media, termostater med −/+, strømpris der fingeren viser prisen for timen, lys med lysstyrke man drar, og gardiner/markise med dra og forhåndsvalg |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-sikkerhetspanel-card.svg" width="28" align="absmiddle"> | `ki-sikkerhetspanel-card` | KI Sikkerhetspanel | Sikkerhetspopupen: ring med én strek per sensor rundt alarmmodusen, hold inne for å bytte modus (med kodetastatur), det som krever deg med en knapp per ting, rommene med sensorene sine, og siste hendelser |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-kart-card.svg" width="28" align="absmiddle"> | `ki-kart-card` | KI Kart | Kartet med sonene, personene og bilen, og under det hvor hver enkelt er, siden når, hvor langt unna hjemme og bilens batteri, pluss sonene med hvem som er der |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-entur-card.svg" width="28" align="absmiddle"> | `ki-entur-card` | KI Entur | Bussen begge veier fra Entur-sensorene: neste avgang som nedtelling, i rute eller forsinket, de neste etter, og når du må gå for å rekke den |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-post-bursdag-card.svg" width="28" align="absmiddle"> | `ki-post-bursdag-card` | KI Post og bursdager | Neste postlevering og bursdag, postdagene to uker fram, pakker på vei, bursdagene med «Legg til», og en månedskalender med alt samlet |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-robot-card.svg" width="28" align="absmiddle"> | `ki-robot-card` | KI Robot | Animert robotklipper eller robotstøvsuger som kjører i baner, med fremdrift, dokking og feil |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-utelys-card.svg" width="28" align="absmiddle"> | `ki-utelys-card` | KI Utelys | Animert himmel med sol i riktig posisjon, stjerner og utelys som tennes |
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-eksempler-card.svg" width="28" align="absmiddle"> | `ki-eksempler-card` | KI Eksempler | Hva hverdagslige ting koster ved dagens strømpris, som en liste i stedet for ett kort per eksempel |
...
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-lansering-card.svg" width="28" align="absmiddle"> | `ki-lansering-card` | KI Lansering | Kommende episoder og filmer fra Sonarr og Radarr, med plakat og bakgrunnsbilde. `plex_serier:` / `plex_filmer:` gir hake på det som alt ligger i Plex, og en Plex-fane med det som nettopp er lagt til. Detaljene åpner midt i skjermen (`detalj_plass: kort` legger dem over kortet som før) |
...
| <img src="https://raw.githubusercontent.com/SebastianKristo/ki-cards/main/brand/ki-tabs-card.svg" width="28" align="absmiddle"> | `ki-tabs-card` | Faner som piller, rullbar rad eller nedtrekksmeny (`style: auto` går fra piller til rullbar rad når de ikke får plass), `sticky: true`, og `default:` velger fanen kortet åpner på – nummeret eller tittelen på fanen, også i den visuelle editoren. `rad_hoyde:` setter fanens høyde i px (lavere enn `fane_hoyde` kommer), og `ikon_naar:` / `kort_naar:` lar en fane bytte ikon og innhold etter hvilken fane som er valgt |
